...
RTRendererAPI `2.x` 是稳定 API 线。Maven Central 是唯一二进制事实源；`vMAJOR.MINOR.PATCH` Git tag 只标识构建该 Central 制品的不可变源码，不附带第二套 GitHub 二进制资产。
...
公开 enum 可以在 minor 版本增加常量。消费方必须为未来值保留 `default` 分支，不能依赖穷举 `switch` 在整个 `2.x` 中维持源码完整性；需要完全封闭的状态域将通过新类型或下一 major 演进。
