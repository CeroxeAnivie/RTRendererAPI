...
- `3.1.4` 是兼容 patch：generic Vulkan command planner 对同一事务中的 buffer upload、自动可见性依赖与后续 descriptor read 作一次完整顺序验证；descriptor materialization 只重新验证 resident generation 和不可变 descriptor，不再错误要求该事务完成后才会产生的跨事务 `GPU_READY` 证据。没有先行 write/visibility 的读取仍保留原有 readiness 门禁，公共 API/SPI 不变。
