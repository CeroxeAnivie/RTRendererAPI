...
Maven 坐标是 `top.ceroxe.rt:renderer-api:4.1.0`。只声明这一个依赖即可；Windows Vulkan 后端、NVIDIA provider 与经过完整性校验的 native runtime 会传递解析。消费方不需要安装 SDK、配置 SDK root 或手工复制 DLL。Maven Central 是这些制品的唯一发布事实源；Git tag 只用于定位构建相同制品的源码。

## 最小调用

`RendererBootstrap.open(RendererPreset.CPU_READBACK)` 会枚举已安装 provider、执行兼容性探测并打开优先级最高的可用后端。兼容目标是 Windows 10 x64 或更高版本、NVIDIA RTX 20 系或更新 GPU、Vulkan 1.2+ 与 Java 21 或更高版本。4.1.0 已在 NVIDIA GeForce RTX 5080 Laptop GPU 上通过发布门与 procedural/triangle RT 验证，详见 [4.1.0 验证记录](quality/4.1.0-procedural-validation.md)。这些结果不代表全部兼容硬件或特定宿主集成都已验收。
...
能力必须逐项查询。`4.1.0` 的 Vulkan 后端真实执行版本化 buffer/texture、view/sampler、staging upload、buffer/texture copy、buffer-image copy、typed clear/barrier、SPIR-V compute/graphics pipeline、render pass、direct/multi/indirect draw，以及在设备支持时执行通用 BLAS/TLAS build、显式 RT shader group/SBT、AS descriptor 和 trace dispatch，并通过 fence evidence 公开完成状态。逻辑操作和 geometry shader 现在分别受设备 feature capability 约束；RT pipeline/SBT 可通过 `RetireRayTracingPipelineCommand` 按完成序列退休。`BindingType.COMBINED_IMAGE_SAMPLER` 必须使用一个 `BindingSet.CombinedImageSamplerValue`，Vulkan 会将同一 view/sampler 对写入一个 `VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER`。接口布局会在 pipeline 创建前与 SPIR-V 逐项核对；错误的 entry point、stage、set/binding、数组长度或拆分 descriptor 声明均 fail-closed。RT 的 `RECORDED` 或 `OUTPUT_PRODUCED` 都不是显示可见证据；能力未标为 executable 的功能不能根据静态类型推断可用。
...
显式覆盖 4.1.0 中保留的全部 NVIDIA 能力，同时保持生产环境可降级：DLSS SR 不可用时允许 NIS，
...
以下片段都只需要 `top.ceroxe.rt:renderer-api:4.1.0`。把对应 options 传给
...
NRD 的 GPUScene 集成遵循官方 REBLUR 输入契约：未命中（天空）像素使用高于
`denoisingRange` 的 `viewZ` sentinel，并在 compose 阶段保留原始 trace；不得将天空编码为
`viewZ = 0`，否则会被 NRD 当作有效表面而复用历史。NRD 的 queued-frame 数与
`maxFramesInFlight` 一致，保证描述符和常量缓冲不会在 GPU 完成前复用。多 SPP 会先以首个有效
表面的 material factor 对每个有效样本做 demodulation，再平均后送入 NRD。
