...
> `renderer-api` 不包含游戏、引擎或 NVIDIA 专用场景字段；3.1.0 同时保留 retained RT 场景快速路径，
...
> AMD、Intel、Linux、macOS、移动平台、D3D12、Metal 与软件渲染器不属于 `3.1.0` 当前后端实现范围。
...
```kotlin
dependencies {
    implementation("top.ceroxe.rt:renderer-api:3.1.0")
}
```
...
A：不能把它们视为 `3.1.0` 的受支持目标。当前兼容范围只包含 Windows x64 与通过运行时 capability gate 的 NVIDIA RTX GPU；具体实机证据以对应提交的验收结果为准。
