...
仓库当前构建的 `top.ceroxe.rt:renderer-api:3.1.12` 及其完整传递运行时。Demo 以 Maven Central 为第一解析源，
...
java -jar .\demos\hex-ball\build\libs\RTRendererAPI-HexBallDemo-3.1.12.jar `
...
```powershell
## 原生基线
java -Ddemo.disable-fg=true -jar .\demos\hex-ball\build\libs\RTRendererAPI-HexBallDemo-3.1.12.jar `
  --width=2560 --height=1440 --spp=2 --duration-seconds=90

## FG 2x
java -Ddemo.fg-multiplier=2 -jar .\demos\hex-ball\build\libs\RTRendererAPI-HexBallDemo-3.1.12.jar `
  --width=2560 --height=1440 --spp=2 --duration-seconds=90

## MFG 3x
java -Ddemo.fg-multiplier=3 -jar .\demos\hex-ball\build\libs\RTRendererAPI-HexBallDemo-3.1.12.jar `
  --width=2560 --height=1440 --spp=2 --duration-seconds=90
```
