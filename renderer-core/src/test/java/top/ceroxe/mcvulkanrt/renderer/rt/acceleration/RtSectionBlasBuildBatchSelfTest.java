...
/** Verifies one immutable metadata identity from queue claim through native BLAS ownership. */
public final class RtSectionBlasBuildBatchSelfTest {
    ...
    private static void claimedIdentityOrderOwnsEveryMetadataField() {
        ...
        RtPendingBlasBuildQueue.Work<RtSectionBlasBuildMetadata> firstWork = work(first, 11L, 0x20);
        RtPendingBlasBuildQueue.Work<RtSectionBlasBuildMetadata> secondWork = work(second, 19L, 0x40);
        List<RtPendingBlasBuildQueue.Work<RtSectionBlasBuildMetadata>> candidates =
                new ArrayList<>(List.of(firstWork, secondWork));
        ...
        require(batch.meshes().equals(List.of(second, first)),
                "batch did not preserve native claim identity order");
        require(batch.workItems().equals(List.of(secondWork, firstWork)),
                "claimed work order diverged from mesh ownership order");
        require(batch.sections().get(0).metadata().contentRevision() == 19L
                        && batch.sections().get(0).metadata().sourceFlags() == 0x40,
                "section metadata drifted from its claimed mesh");
        require(batch.sections().get(1).metadata().contentRevision() == 11L
                        && batch.sections().get(1).metadata().sourceFlags() == 0x20,
                "second claimed section metadata drifted");
        require(batch.retainedEstimatedBytes()
                        == Math.addExact(first.estimatedBytes(), second.estimatedBytes()),
                "batch byte retention did not cover every claimed mesh");
    ...
    private static void malformedClaimsFailBeforeNativeRecording() {
        ...
        expectIllegalArgument(() -> RtSectionBlasBuildBatch.capture(
                List.of(work, work),
                List.of(first)
        ));
        expectIllegalArgument(() -> RtSectionBlasBuildBatch.capture(
    ...
    private static void metadataRejectsInvalidGenerations() {
        ...
        RtSceneMaterialTable.SectionMaterial material = RtSceneMaterialTable.SectionMaterial.fromMesh(mesh);
        expectIllegalArgument(() -> new RtSectionBlasBuildMetadata(
                -1L,
                RendererFrameCausality.untraced(1L),
                0,
                material
        ));
        expectNullPointer(() -> new RtSectionBlasBuildMetadata(1L, null, 0, material));
        expectNullPointer(() -> new RtSectionBlasBuildMetadata(
                1L,
                RendererFrameCausality.untraced(1L),
                0,
                null
    ...
    private static RtPendingBlasBuildQueue.Work<RtSectionBlasBuildMetadata> work(
            SectionTriangleMesh mesh,
            long revision,
            int sourceFlags
    ) {
        return new RtPendingBlasBuildQueue.Work<>(
                mesh,
                new RtSectionBlasBuildMetadata(
                        revision,
                        RendererFrameCausality.untraced(revision),
                        sourceFlags,
