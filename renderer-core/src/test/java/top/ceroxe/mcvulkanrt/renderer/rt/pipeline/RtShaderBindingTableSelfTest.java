...
/**
 * Validates the pipeline-owned shader binding table ABI without bringing scene,
 * material, acceleration-structure, or device-lifetime state into the test.
 */
public final class RtShaderBindingTableSelfTest {
    ...
    private static void computesShaderBindingTableLayoutWithBaseAlignment() {
        RtRayTracingPipelineProperties properties = new RtRayTracingPipelineProperties(
                32,
                32,
                64,
                4096,
        ...
        RtRayTracingPipelineProperties.ShaderBindingTableLayout layout =
                properties.shaderBindingTableLayout(1, 2, 3, 0);
        ...
        byte[] handles = new byte[6 * 32];
        for (int group = 0; group < 6; group++) {
            handles[group * 32] = (byte) (group + 1);
        }
        RtRayTracingPipelineProperties.ShaderBindingTableData table =
                properties.packShaderGroupHandles(handles, 1, 2, 3, 0);
