...
/** Verifies paired lifecycle publications, revision ownership, and fail-fast active invariants. */
public final class RtSectionLifecycleMembershipStateSelfTest {
    ...
    public static void main(String[] arguments) {
        ...
        state.addResident(first);
        require(state.residentRevision() == residentRevision,
                "duplicate resident insertion must not advance membership revision");
        ...
        require(active.containsAll(List.of(first, second)) && state.activeRevision() == 2L,
                "batched active changes were not committed in one exact publication");
        require(state.active() == active,
                "stable active revision must reuse its immutable publication identity");
        expectFailure(() -> state.addActive(first));
        state.removeActive(first);
        state.removeActive(second);
        require(state.active().isEmpty() && state.activeRevision() == 4L,
                "batched active removals did not publish an empty successor");
        ...
        require(bound.size() == 2 && state.bound(boundInput, 9L) == bound,
                "stable bound-world revision must reuse its immutable publication");
        state.clearResidents();
        require(!state.hasResidents() && state.resident().isEmpty(),
                "resident clear did not invalidate and rebuild the publication");
