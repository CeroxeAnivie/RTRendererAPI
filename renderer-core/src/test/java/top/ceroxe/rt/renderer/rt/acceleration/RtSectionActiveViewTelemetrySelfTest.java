...
import top.ceroxe.rt.renderer.rt.acceleration.RtSectionTlasBuildInputCache.Stats;
...
public final class RtSectionActiveViewTelemetrySelfTest {
   ...
   public static void main(String[] arguments) {
      ...
      telemetry.addAdmissionNanos(12345L);
      telemetry.assembly(4567L, 8901L, true, 2, 1);
      ...
      require(sink.details.contains("hits=1") && sink.details.contains("rebuilds=1"), "active-view cache deltas were not published");
      require(sink.details.contains("identityDelta={changes=1, added=2, removed=1}"), "active-view identity deltas were not published");
      require(sink.details.contains("sourceSections=4") && sink.details.contains("requiresView=true"), "telemetry must report the window-ending admission state, not the previous baseline");
      System.out.println("RtSectionActiveViewTelemetrySelfTest passed");
   }

   private static RtSectionActiveViewTelemetry.Sample sample(long planTotals, int sourceSections, boolean requiresView) {
      return new RtSectionActiveViewTelemetry.Sample(planTotals, planTotals, planTotals, planTotals, planTotals, planTotals, planTotals, Stats.empty(), sourceSections, 8, requiresView, requiresView, false, false, false);
   }
