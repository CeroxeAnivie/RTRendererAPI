...
import top.ceroxe.rt.renderer.rt.acceleration.RtSectionOwnershipProof.Classification;
...
public final class RtActiveSectionContentStateSelfTest {
   ...
   public static void main(String[] arguments) {
      ...
      require(first.causalities().causality(key).equals(firstCausality), "active causality was not aligned with its revision");
      long stableGeneration = state.generation();
      state.install(key, 41L, firstCausality);
      require(state.generation() == stableGeneration, "an identical install must not create a false content generation");
      require(first == state.publication(membership), "an identical install must preserve publication identity");
      ...
      require(successor.revisions().valueOrDefault(key, -1L) == 42L, "successor revision was not published");
      require(successor.causalities().causality(key).equals(secondCausality), "successor causality was not published atomically");
      state.remove(key);
      require(state.publication(PackedSectionMembership.empty()).revisions().isEmpty(), "removing the last live BLAS must release the derived publication");
      require(state.firstMissingRevision(membership).equals(key), "failure-only diagnosis must identify the exact missing publication key");
      RtSectionOwnershipProof.Presence divergentPresence = new RtSectionOwnershipProof.Presence(true, true, true, false, true, true, true, false, false, false, false, true);
      require(RtSectionOwnershipProof.classify(divergentPresence) == Classification.ACTIVE_MEMBERSHIP_WITHOUT_CONTENT, "proof classifier must distinguish active membership/content divergence");
