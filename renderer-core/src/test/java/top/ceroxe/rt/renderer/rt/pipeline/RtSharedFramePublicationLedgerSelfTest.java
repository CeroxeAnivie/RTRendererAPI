...
public final class RtSharedFramePublicationLedgerSelfTest {
   ...
   private static void acknowledgementPromotesOnlyTheCurrentCompletedSceneProof() {
      ...
      require(!ledger.presentedState().available(), "a retired slot without the current completion proof must not publish stale scene coverage");
   }
   ...
   private static void presentedFrontCanBeReservedAfterNewCompletion() {
      ...
      require(reservation.exported() == presented, "the acknowledged front must remain exportable while a successor is incomplete");
      require(ledger.retainsForPublication(presented), "reserving the acknowledged front must retain its image lifetime");
   }
