...
public final class RtFirstFrontBlasFlightRecorderSelfTest {
   ...
   private static void disabledRecorderIsAllocationAndClockSilent() {
      ...
      RtFirstFrontBlasFlightRecorder recorder = new RtFirstFrontBlasFlightRecorder(false, 2, () -> {
   ...
   private static void boundedTraceFiltersSectionsAndReportsOverwrite() {
      ...
      Objects.requireNonNull(elapsedMillis);
      RtFirstFrontBlasFlightRecorder recorder = new RtFirstFrontBlasFlightRecorder(true, 2, elapsedMillis::getAndIncrement);
      SectionKey discarded = new SectionKey(1, 2, 3);
      SectionKey foreground = new SectionKey(4, 5, 6);
      recorder.record("queued", discarded, 7L, 8L, 6L, 17, false, false, false);
      recorder.record("gpuSubmitted", foreground, 9L, 10L, 8L, 42, true, true, false);
      ...
      String details10000 = dump.trace();
      String details10001 = String.valueOf(foreground);
      require(details10000.contains("key=" + details10001), "trace must identify the foreground section");
      require(dump.trace().contains("3@12ms:progress"), "trace must retain aggregate progress");
      details10000 = dump.trace();
      details10001 = String.valueOf(discarded);
      require(!details10000.contains("key=" + details10001), "trace must filter non-foreground sections");
   ...
   private static void invalidConstructionAndPublicationInputsFailEarly() {
      require(expectFailure(() -> new RtFirstFrontBlasFlightRecorder(true, 0, () -> 0L)) instanceof IllegalArgumentException, "zero ring capacity must be rejected");
      require(expectFailure(() -> new RtFirstFrontBlasFlightRecorder(true, 1, (LongSupplier)null)) instanceof NullPointerException, "missing elapsed-time source must be rejected");
      RtFirstFrontBlasFlightRecorder recorder = new RtFirstFrontBlasFlightRecorder(true, 1, () -> 0L);
      require(expectFailure(() -> recorder.dumpOnce(null)) instanceof NullPointerException, "enabled publication must reject a missing foreground set");
   }
