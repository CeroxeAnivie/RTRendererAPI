...
public final class RtFrameDispatchAdmissionSelfTest {
   ...
   private static void presentationGateWinsBeforeAllOtherReasons() {
      RtFrameDispatchAdmission.Decision decision = decide(true, false, false, false, true, 4, 4, 4);
      require(decision == Decision.PRESENTATION_ELIGIBILITY_GATE, "presentation eligibility must be the first rejection reason");
   ...
   private static void invalidFrameWinsBeforeCadenceAndBacklog() {
      RtFrameDispatchAdmission.Decision decision = decide(false, false, true, false, true, 4, 4, 4);
      require(decision == Decision.INVALID_FRAME_STATE, "invalid frame state must be rejected before cadence and backlog");
   ...
   private static void cadenceWinsBeforeBacklog() {
      RtFrameDispatchAdmission.Decision decision = decide(false, true, true, true, true, 4, 4, 4);
      require(decision == Decision.DISPATCH_INTERVAL, "dispatch cadence must be evaluated before pending submissions");
      require(decision.incrementsIntervalSkips(), "cadence counter must be marked");
   }

   private static void backlogWinsBeforeRingCapacity() {
      RtFrameDispatchAdmission.Decision decision = decide(false, true, true, true, false, 4, 4, 8);
      require(decision == Decision.MAX_PENDING_SUBMISSIONS, "configured pending limit must be reported before physical ring capacity");
   ...
   private static void ringCapacityIsTheLastAdmissionRejector() {
      RtFrameDispatchAdmission.Decision decision = decide(false, true, true, true, false, 4, 8, 4);
      require(decision == Decision.FRAME_SLOT_RING_BUSY, "physical frame ring capacity must be the final admission rejection");
   ...
   private static void acceptsOnlyWhenEveryGateIsClear() {
      RtFrameDispatchAdmission.Decision decision = decide(true, true, false, true, false, 3, 4, 4);
      require(decision == Decision.ACCEPTED, "an eligible valid frame with capacity must be accepted");
   ...
   private static RtFrameDispatchAdmission.Decision decide(boolean requiresPresentationEligibility, boolean presentationEligible, boolean presentationProbeDue, boolean frameStateValid, boolean intervalBlocked, int pendingSubmissionCount, int maxPendingSubmissions, int frameSlotCount) {
      return RtFrameDispatchAdmission.decide(new RtFrameDispatchAdmission.State(requiresPresentationEligibility, presentationEligible, presentationProbeDue, frameStateValid, intervalBlocked, pendingSubmissionCount, maxPendingSubmissions, frameSlotCount));
   }
