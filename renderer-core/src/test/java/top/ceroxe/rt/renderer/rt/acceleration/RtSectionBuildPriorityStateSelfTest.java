...
public final class RtSectionBuildPriorityStateSelfTest {
   ...
   public static void main(String[] arguments) {
      ...
      require(provisional.equals(Set.of(first, second)), "first source batch did not establish provisional priority");
      expectFailure(() -> provisional.add(later));
      state.admitProvisional(false, List.of(later));
      require(state.preferredKeys() == provisional, "later source batch replaced the stable provisional publication");
      state.markInteractive(second);
      state.markInteractive(second);
      require(state.isInteractive(second) && state.interactiveCount() == 1, "interactive urgency was not idempotent");
      ...
      require(state.preferredKeys() == authority, "authoritative immutable membership identity was not preserved");
      require(!state.preferredKeys().contains(first), "provisional keys survived authoritative priority promotion");
      state.admitProvisional(true, List.of(first));
      require(state.preferredKeys() == authority, "authoritative priority was replaced by a later provisional batch");
      ...
      state.clear();
      require(state.preferredKeys().isEmpty() && state.interactiveKeys().isEmpty(), "full priority reset did not clear both lanes");
   ...
   private static void expectFailure(Runnable action) {
      try {
         action.run();
      } catch (UnsupportedOperationException value2) {
         return;
      }
