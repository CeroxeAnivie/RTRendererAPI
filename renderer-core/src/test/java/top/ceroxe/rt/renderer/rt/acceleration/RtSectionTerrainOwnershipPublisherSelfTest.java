...
public final class RtSectionTerrainOwnershipPublisherSelfTest {
   ...
   private static void stableRevisionVectorReusesSnapshotIdentity() {
      RtSectionSourceStore sources = new RtSectionSourceStore(1000000L);
      RtPendingBlasBuildQueue<Void> queued = new RtPendingBlasBuildQueue<>(8, 1000000L);
      ...
      RtSectionTerrainOwnershipPublisher publisher = new RtSectionTerrainOwnershipPublisher(sources, queued, async, lifecycle, foreground);
      ...
      try {
         ...
         require(first.ownershipGeneration() == 0L && publisher.generation(-1L) == 0L, "initial ownership generation drifted");
         SectionTriangleMesh mesh = mesh(4);
         queued.enqueue(mesh);
         require(publisher.generation(-1L) == 1L, "queued membership revision did not advance ownership generation");
         NativeTerrainOwnership queuedSnapshot = publisher.snapshot(PackedSectionMembership.empty(), -1L, true);
         require(queuedSnapshot.queuedSectionKeys().contains(mesh.key()), "queued ownership was omitted from the native snapshot");
         require(queuedSnapshot.ownershipGeneration() == 1L, "snapshot did not reuse the already-observed scalar generation");
         lifecycle.addActive(mesh.key());
         PackedSectionMembership bound = PackedSectionMembership.copyOf(Set.of(mesh.key()));
         NativeTerrainOwnership activeAndBound = publisher.snapshot(bound, 7L, true);
         require(activeAndBound.activeSectionKeys().contains(mesh.key()), "active lifecycle publication was not projected");
         require(activeAndBound.boundSectionKeys().contains(mesh.key()), "bound lifecycle publication was not projected");
         require(activeAndBound.ownershipGeneration() == 2L, "one composite revision-vector change advanced generation more than once");
      } finally {
         RuntimeException failure = async.closeCollecting((RuntimeException)null);
         if (failure != null) {
            throw failure;
         }
   ...
   private static void invalidBoundRevisionFailsBeforePublication() {
      ...
      try {
         RtSectionTerrainOwnershipPublisher publisher = new RtSectionTerrainOwnershipPublisher(new RtSectionSourceStore(1000000L), new RtPendingBlasBuildQueue<>(), async, new RtSectionLifecycleMembershipState(), new RtSectionForegroundState());
