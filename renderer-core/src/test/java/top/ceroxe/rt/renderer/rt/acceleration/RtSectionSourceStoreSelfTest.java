...
public final class RtSectionSourceStoreSelfTest {
   ...
   private static void observationDoesNotChangePayloadEvictionOrder() {
      ...
      RtSectionSourceStore store = new RtSectionSourceStore(Math.multiplyExact(basePayloadBytes(first), 2L));
      publish(store, first);
      publish(store, second);
      require(store.publicationsView().get(first.key()) != null, "first publication is missing");
      publish(store, third);
      RtSectionSourceStore.TrimResult trim = store.trimToBudget((SectionKey)null, Set.of());
      require(trim.releasedPayloads() == 1 && !trim.overBudget(), "one oldest payload should restore the source budget");
      require(store.mesh(first.key()) == null, "observational read changed payload eviction order");
      require(store.mesh(second.key()) != null && store.mesh(third.key()) != null, "newer payloads must remain resident");
   }
   ...
   private static void clearInvalidatesFarFieldOnlyMembership() {
      ...
      require(store.payloadCount() == 0 && !store.membership().isEmpty(), "compact FarField source must outlive the heavyweight Base payload");
      RtSectionSourcePublication compact = store.publication(mesh.key());
      require(compact.contentRevision() == 7L && compact.causality().equals(RendererFrameCausality.untraced(7L)), "compact FarField source must retain its captured content revision and causality");
      long beforeClear = store.membershipRevision();
      store.clear();
      require(store.membershipRevision() == beforeClear + 1L, "clearing a FarField-only publication must advance membership");
   ...
   private static void failedPublicationDoesNotCommitAccounting() {
      ...
      SectionTriangleMesh invalid = new SectionTriangleMesh(new SectionKey(10, 0, 0), valid.vertexPositions(), valid.indices(), valid.faceVoxelStateIds(), valid.faceFluidAmounts(), new byte[]{127});
      RtSceneMaterialTable.SectionMaterial material = SectionMaterial.fromMesh(valid);
      expectFailure(() -> store.publish(invalid, material, 10L, RendererFrameCausality.untraced(10L), false, true));
      require(store.publicationCount() == 0 && store.payloadCount() == 0 && store.payloadBytes() == 0L, "failed source construction partially committed store accounting");
      require(store.membershipRevision() == 0L, "failed source construction advanced membership revision");
   }

   private static void publish(RtSectionSourceStore store, SectionTriangleMesh mesh) {
      store.publish(mesh, SectionMaterial.fromMesh(mesh), (long)mesh.key().x(), RendererFrameCausality.untraced((long)mesh.key().x()), false, true);
   }

   private static long basePayloadBytes(SectionTriangleMesh mesh) {
      return Math.addExact(mesh.estimatedBytes(), (long)SectionMaterial.fromMesh(mesh).estimatedBytes());
   }
