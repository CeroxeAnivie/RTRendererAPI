...
final class RtAsyncFrameDispatchRecorder {
    ...
    private static int requireOutputLayout(int layout) {
        return switch (layout) {
            case VK10.VK_IMAGE_LAYOUT_UNDEFINED, VK10.VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK10.VK_IMAGE_LAYOUT_GENERAL -> layout;
    ...
    private static int outputSourceAccessMask(int outputImageLayout) {
        return switch (outputImageLayout) {
            ...
            default -> throw new IllegalArgumentException("unsupported RT presentation output image layout: " + outputImageLayout);
        };
    }
