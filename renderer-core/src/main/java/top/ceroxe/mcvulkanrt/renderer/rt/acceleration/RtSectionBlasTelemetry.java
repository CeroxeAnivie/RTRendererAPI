...
final class RtSectionBlasTelemetry {
    ...
    void recordingPoll(long micros) { budget.recordingPollMicros += micros; }
    void recordingScan(long micros) { budget.recordingScanMicros += micros; }
    void recordingComplete(long micros) { budget.recordingCompleteMicros += micros; }
    void recordingNativeSubmit(long micros) { budget.recordingNativeSubmitMicros += micros; }
    void gpuPollApply(long micros) { budget.gpuPollApplyMicros += micros; }
    void gpuPoll(long micros) { budget.gpuPollMicros += micros; }
    void gpuApply(long micros) { budget.gpuApplyMicros += micros; }
    void coverageLimits(long micros) { budget.coverageLimitsMicros += micros; }
    void capacity(long micros) { budget.capacityMicros += micros; }
    void drain(long micros) { budget.drainMicros += micros; }
    void recordingEnqueue(long micros) { budget.recordingEnqueueMicros += micros; }
    void bookkeeping(long micros) { budget.bookkeepingMicros += micros; }
    void farField(long micros) { budget.farFieldMicros += micros; }
