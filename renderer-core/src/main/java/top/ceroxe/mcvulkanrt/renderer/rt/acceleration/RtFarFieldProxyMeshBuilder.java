...
/**
 * Builds a bounded directional-depth proxy for one FarField admission cell.
 *
 * <p>Each source face votes into an orthographic bucket for its sourceEngine face
 * direction. The outermost face wins and is expanded to the bucket footprint.
 * This preserves terrain/building silhouettes from all six directions while
 * keeping proxy complexity independent of the number of source triangles. Face
 * references remain explicit so proxy materials can be generated from the
 * authoritative source face without coupling geometry reduction to the current
 * GPU material layout.</p>
 */
/**
 * Builds the coarse far-field geometry and preserves its exact source-face
 * mapping so material reconstruction never needs to reach into BLAS state.
 */
