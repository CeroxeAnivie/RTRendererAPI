...
final class RtWorldSceneBindStatistics {
    ...
    void worldSubmitted() { worldSubmissions++; }
    void worldCompleted() { worldCompletions++; }
    void worldPollNotReady() { worldPollsNotReady++; }
    void worldDescriptorDeferred() { worldDescriptorDeferrals++; }
    void worldReplaced() { worldReplacements++; }
    void materialSubmitted() { materialSubmissions++; }
    void materialCompleted() { materialCompletions++; }
    void materialPollNotReady() { materialPollsNotReady++; }
    void materialDescriptorDeferred() { materialDescriptorDeferrals++; }
    void materialSkippedNoBoundWorld() { materialSkippedNoBoundWorld++; }
    void materialSkippedPendingWorld() { materialSkippedPendingWorld++; }
    void materialSkippedUnchanged() { materialSkippedUnchanged++; }
    void deferredWorldCreated() { deferredWorldBinds++; }
    void deferredWorldDeferred() { deferredWorldDeferrals++; }
    void deferredWorldSubmitted() { submittedDeferredWorldBinds++; }
    void deferredWorldDiscarded() { discardedDeferredWorldBinds++; }
    void coverageProtectedDiscarded() { coverageProtectedDiscards++; }
    void streamingIntervalDeferred() { streamingIntervalDeferrals++; }
