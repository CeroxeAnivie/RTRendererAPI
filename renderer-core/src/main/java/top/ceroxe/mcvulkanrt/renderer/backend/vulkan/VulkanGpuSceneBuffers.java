...
final class VulkanGpuSceneBuffers implements VulkanGpuSceneTransferQueue {
    ...
    public synchronized void close() {
        ...
        if (pending != null) {
            if (pending.submission != null) {
                try { pending.submission.close(); } catch (RuntimeException ex) { failure = collect(failure, ex); }
            }
            if (pending.staging != null) {
                try { pending.staging.close(); } catch (RuntimeException ex) { failure = collect(failure, ex); }
            }
            failure = closeNewTargets(pending.targets, pending.newlyAllocated, failure);
            pending = null;
        }
        for (RtGpuBuffer buffer : active.values()) {
            try { buffer.close(); } catch (RuntimeException ex) { failure = collect(failure, ex); }
        }
        active.clear();
        try { descriptorFallback.close(); } catch (RuntimeException ex) { failure = collect(failure, ex); }
        for (RetiredBuffer entry : retired) {
            try { entry.buffer().close(); } catch (RuntimeException ex) { failure = collect(failure, ex); }
        }
        retired.clear();
        if (failure != null) throw failure;
    }
    ...
    private static RuntimeException closeNewTargets(
        ...
        for (VulkanGpuSceneUploadPlanner.Target target : newlyAllocated) {
            try { targets.get(target).close(); } catch (RuntimeException ex) { result = collect(result, ex); }
        }
        return result;
    }
