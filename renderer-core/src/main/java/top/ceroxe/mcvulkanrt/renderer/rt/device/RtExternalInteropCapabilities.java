...
final class RtExternalInteropCapabilities {
    ...
    boolean gpuInteropCandidate() { return support.gpuInteropCandidate(); }
    boolean dedicatedOnly() { return support.dedicatedOnly(); }
    String reason() { return support.reason(); }
    VulkanWin32ExternalMemoryProbe.Result memoryProbe() { return memoryProbe; }
    VulkanWin32ExternalSemaphoreProbe.Result semaphoreProbe() { return semaphoreProbe; }
    ...
    private static Support querySupport(
        ...
        VkPhysicalDeviceImageFormatInfo2 imageFormatInfo = VkPhysicalDeviceImageFormatInfo2.calloc(stack)
                .sType$Default().pNext(externalInfo).format(RtRayTracingPipeline.bootstrapOutputFormat())
