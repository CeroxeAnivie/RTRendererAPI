...
final class VulkanFrameFlightRecorder {
    ...
    private static final boolean ENABLED = Boolean.getBoolean("top.ceroxe.rt.renderer.jfr.enabled");
    private static final long MAX_EVENTS = positiveLongProperty(
            "top.ceroxe.rt.renderer.jfr.frameMaxEvents", 262_144L
    );
    ...
    @Name("top.ceroxe.rt.VulkanFrameLifecycle")
    @Label("Vulkan RT Frame Lifecycle")
    @Category({"RTRenderer", "Renderer", "Frame"})
    @StackTrace(false)
    ...
    @Name("top.ceroxe.rt.VulkanFrameCaptureLoss")
