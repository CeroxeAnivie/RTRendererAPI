 ...
 * snapshot to an isolated validation process.</p>
 */
public final class SectionVoxelSnapshotCodec {
    private static final int MAGIC = 0x52545653; // RTVS
     ...
     * 将区段快照写入稳定的有界二进制格式。
     *
     * <p>该方法不会关闭调用方拥有的输出流，但会在返回前刷新已写入的数据。</p>
     *
     * @param snapshot 要编码的不可变快照
     * @param output   接收编码结果的输出流
     ...
     * 从稳定二进制格式读取一个区段快照。
     *
     * <p>所有数组长度均由协议版本固定，不接受输入流声明的任意分配长度。该方法不会关闭输入流。</p>
     *
     * @param input 提供编码数据的输入流
     * @return 完整校验并冻结后的区段快照
     * @throws IOException 输入截断、魔数或版本不受支持、或读取失败时抛出
     */
    public static SectionVoxelSnapshot read(InputStream input) throws IOException {
        Objects.requireNonNull(input, "input");
        DataInputStream data = new DataInputStream(input);
        if (data.readInt() != MAGIC) throw new IOException("not an RTRenderer section snapshot");
    ...
    private static void writeBoundary(DataOutputStream data, SectionBoundarySnapshot boundary) throws IOException {
        ...
        writeInts(data, blockStates);
        writeInts(data, fluidVoxelStates);
        writeInts(data, mediumTypes);
        writeBytes(data, mediumAmounts);
        writeBytes(data, geometryFlags);
        writeInts(data, mapColors);
        writeBytes(data, emissions);
        writeBytes(data, lightFlags);
        writeBytes(data, shades);
    }
    ...
    private static void forEachGeometry(CoordinateConsumer consumer) {
        for (int y = -1; y <= SectionVoxelSnapshot.SECTION_SIZE; y++)
            for (int z = -1; z <= SectionVoxelSnapshot.SECTION_SIZE; z++)
                for (int x = -1; x <= SectionVoxelSnapshot.SECTION_SIZE; x++)
                    if (SectionBoundarySnapshot.containsGeometryCoordinate(x, y, z))
                        consumer.accept(x, y, z, SectionBoundarySnapshot.geometrySampleIndex(x, y, z));
    }

    private static void forEachLight(CoordinateConsumer consumer) {
        for (int y = -2; y <= SectionVoxelSnapshot.SECTION_SIZE + 1; y++)
            for (int z = -2; z <= SectionVoxelSnapshot.SECTION_SIZE + 1; z++)
                for (int x = -2; x <= SectionVoxelSnapshot.SECTION_SIZE + 1; x++)
                    if (SectionBoundarySnapshot.containsLightCoordinate(x, y, z))
                        consumer.accept(x, y, z, SectionBoundarySnapshot.lightSampleIndex(x, y, z));
    }

    @FunctionalInterface
    private interface CoordinateConsumer {
        void accept(int x, int y, int z, int index);
    }
}
