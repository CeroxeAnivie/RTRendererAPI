...
final class RtFrameDispatchFlightRecorder {
    ...
    private static final long MAX_EVENTS = positiveLongProperty(
            "top.ceroxe.rt.takeoverFlightRecorder.frameDispatchMaxEvents",
            262_144L
    );
    ...
    @Name("top.ceroxe.rt.FrameDispatch")
    @Label("RTRenderer RT Frame Dispatch")
    @Category({"RTRenderer", "Renderer", "Dispatch"})
    @StackTrace(false)
    ...
    @Name("top.ceroxe.rt.FrameDispatchCaptureLoss")
    @Label("RTRenderer RT Frame Dispatch Capture Loss")
    @Category({"RTRenderer", "Renderer", "Dispatch"})
    @StackTrace(false)
