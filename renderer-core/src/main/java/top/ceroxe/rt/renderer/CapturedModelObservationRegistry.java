...
/**
 * Owns the short-lived producer cache used while host application captures dynamic
 * model submissions.
 *
...
final class CapturedModelObservationRegistry {
    ...
    private static final class CaptureModelSlot implements DynamicRenderScene.DynamicModelObservation {
        ...
        private DynamicRenderScene.DynamicModelObservation stage(
            ...
            Objects.requireNonNull(transform, "transform");
            beginStage(kind, asset, faceMaterials, packedLight, debugName, renderLane, epoch);
            stagedTransform[0] = transform.m00();
            stagedTransform[1] = transform.m10();
            stagedTransform[2] = transform.m20();
            stagedTransform[3] = transform.m30();
            stagedTransform[4] = transform.m01();
            stagedTransform[5] = transform.m11();
            stagedTransform[6] = transform.m21();
            stagedTransform[7] = transform.m31();
            stagedTransform[8] = transform.m02();
            stagedTransform[9] = transform.m12();
            stagedTransform[10] = transform.m22();
            stagedTransform[11] = transform.m32();
            return this;
        }
        ...
        @Override
        public DynamicRenderScene.PrimitiveKind kind() {
            return staged ? stagedKind : instance().kind();
        }

        @Override
        public DynamicMeshAsset asset() {
            return staged ? stagedAsset : instance().asset();
        }

        @Override
        public List<DynamicMeshInstance.FaceMaterial> faceMaterials() {
            return staged ? stagedMaterials : instance().faceMaterials();
        }

        @Override
        public DynamicRenderLane renderLane() {
            return staged ? stagedRenderLane : instance().renderLane();
        }

        @Override
        public int packedLight() {
            return staged ? stagedPackedLight : instance().packedLight();
        }

        @Override
        public String debugName() {
            return staged ? stagedDebugName : instance().debugName();
        }
