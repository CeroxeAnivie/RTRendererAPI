...
final class SectionFaceStaging {
    ...
    int x(int faceIndex) {
        return valueAt(faceIndex, X);
    }

    int y(int faceIndex) {
        return valueAt(faceIndex, Y);
    }

    int z(int faceIndex) {
        return valueAt(faceIndex, Z);
    }

    FaceDirection direction(int faceIndex) {
        return DIRECTIONS[valueAt(faceIndex, DIRECTION)];
    }

    int voxelTypeId(int faceIndex) {
        return valueAt(faceIndex, BLOCK_STATE);
    }

    int mediumAmount(int faceIndex) {
        return valueAt(faceIndex, FLUID_AMOUNT);
    }

    int mapColor(int faceIndex) {
        return valueAt(faceIndex, MAP_COLOR);
    }
    ...
    int lightEmission(int faceIndex) {
        return valueAt(faceIndex, LIGHT_EMISSION);
    }

    int materialFlags(int faceIndex) {
        return valueAt(faceIndex, MATERIAL_FLAGS);
    }

    boolean faceVisible(int faceIndex) {
        return valueAt(faceIndex, FACE_VISIBLE) != 0;
    }
    ...
    int fluidFlowX(int faceIndex) {
        return valueAt(faceIndex, FLUID_FLOW_X);
    }

    int fluidFlowZ(int faceIndex) {
        return valueAt(faceIndex, FLUID_FLOW_Z);
    }

    boolean fluidOverlay(int faceIndex) {
        return valueAt(faceIndex, FLUID_OVERLAY) != 0;
    }

    int mediumTypeId(int faceIndex) {
        return valueAt(faceIndex, FLUID_TYPE);
    }
