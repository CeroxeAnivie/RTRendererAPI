...
final class VulkanAcceptFrameTiming {
    ...
    String admissionSummary() {
        return "framePendingBuildBudgetDeferrals=" + framePendingBuildBudgetDeferrals
    ...
    void recordPreBuildDispatchOutcome(boolean dispatched, boolean allowed) {
        if (dispatched) {
    ...
    void recordBuildBudgetDeferral() {
        framePendingBuildBudgetDeferrals++;
    }

    void recordForegroundBudgetDeferral() {
        foregroundFrameBudgetDeferrals++;
    }

    void recordStableFrameFastPathDispatch() {
        stableFrameFastPathDispatches++;
    }

    void recordStableFramePendingGpuSkip() {
        stableFramePendingGpuSkips++;
    }

    void recordDynamicWorldTlasDispatchBlock(String reason) {
        String requiredReason = Objects.requireNonNull(reason, "reason");
        if (requiredReason.isBlank()) {
            throw new IllegalArgumentException("reason must not be blank");
        }
        dynamicWorldTlasDispatchBlocks++;
        lastDynamicWorldTlasDispatchBlockReason = requiredReason;
    }
