...
public final class RtTakeoverTimeline {
    /**
     * Event kind for a causality edge.
     */
    public static final int KIND_EDGE = 1;
    /**
     * Event kind for a rate-limited smoke aggregate.
     */
    public static final int KIND_SMOKE_AGGREGATE = 2;
    /**
     * Event kind for a host-side command wait.
     */
    ...
    private static final boolean VERBOSE_IO_ENABLED =
            Boolean.getBoolean("top.ceroxe.rt.takeoverFlightRecorder.verboseIo");
    ...
    /**
     * Returns monotonic milliseconds since diagnostics initialization.
     *
    ...
    /**
     * Clears process-local edge de-duplication state when edge logging is enabled.
     */
    ...
    /**
     * Records an edge, de-duplicating its edge label unless verbose I/O is enabled.
     *
     * @param edge    stable edge label
     * @param details diagnostic payload used only as a fingerprint
     */
    ...
    /**
     * Records an edge at most once for the supplied process-local key.
     *
     ...
     *
     * @param aggregate stable aggregate label
     * @param details   diagnostic payload used only as a fingerprint
     */
     ...
     *
     * @param thread      producer thread label
     * @param commandPool native command-pool handle associated with the wait
     * @param stage       stable wait-stage label
     * @param waitNanos   host wait duration in nanoseconds
     * @param workNanos   associated work duration in nanoseconds
     */
    ...
    /**
     * Reports whether repeated diagnostic I/O is enabled.
     *
     * @return {@code true} when verbose diagnostic I/O is enabled
     */
     ...
     *
     * @return {@code true} when smoke edge recording is enabled
     */
    public static boolean edgeLoggingEnabled() {
        return EDGE_LOGGING_ENABLED;
    }

    @Name("top.ceroxe.rt.RtTakeoverTimeline")
    @Label("RTRenderer RT Takeover Timeline")
