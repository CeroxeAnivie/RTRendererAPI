...
public final class RtGBufferSnapshotCodec {
    private static final int MAGIC = 0x52544742; // RTGB
    ...
    /**
     * Writes one complete snapshot without closing the caller-owned stream.
     *
     * @param snapshot snapshot to serialize
     * @param output   destination stream; flushed but not closed
     * @throws IOException if the stream cannot accept the complete artifact
     */
    ...
    /**
     * Reads one versioned snapshot without closing the caller-owned stream.
     *
     * @param input artifact source
     * @return validated immutable snapshot
     * @throws IOException if the artifact is truncated, invalid, or uses an unsupported version
     */
    public static RtGBufferSnapshot read(InputStream input) throws IOException {
        ...
        if (data.readInt() != MAGIC) throw new IOException("not an RTRenderer G-buffer artifact");
