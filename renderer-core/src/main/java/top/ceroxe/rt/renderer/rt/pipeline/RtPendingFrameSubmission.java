...
final class RtPendingFrameSubmission {
    ...
    long observedFrameStatesAtSubmit() {
        return observedFrameStatesAtSubmit;
    }
    ...
    int width() {
        return outputImage.width();
    }

    int height() {
        return outputImage.height();
    }

    long readbackBytes() {
        return (long) outputImage.width() * outputImage.height() * Integer.BYTES;
    }

    int gBufferWidth() {
        return frameSlot.traceImage().width();
    }

    int gBufferHeight() {
        return frameSlot.traceImage().height();
    }
}
