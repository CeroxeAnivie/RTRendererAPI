...
final class RtDynamicInstanceFlightRecorder {
    ...
    private static final long MAX_EVENTS = positiveLongProperty(
            "top.ceroxe.rt.takeoverFlightRecorder.dynamicInstanceMaxEvents",
            131_072L
    );
    ...
    @Name("top.ceroxe.rt.DynamicTlasInstance")
    @Label("RTRenderer Dynamic TLAS Instance")
    @Category({"RTRenderer", "Dynamic Acceleration"})
    @StackTrace(false)
    ...
    @Name("top.ceroxe.rt.DynamicTlasCaptureLoss")
    @Label("RTRenderer Dynamic TLAS Capture Loss")
    @Category({"RTRenderer", "Dynamic Acceleration"})
    @StackTrace(false)
