 ...
 * lane is inferred from missing fields or translated into the other.</p>
 */
public interface Renderer extends AutoCloseable {
     ...
     *
     * <p>Scene workloads use the retained scene lane, graphics workloads use the generic command
     * lane, and combined workloads are rejected unless the provider has a real ordered composition
     * implementation. No mode is inferred from absent fields.</p>
     *
    ...
    default WorkloadExecutionEvidence submitWorkload(RenderWorkload workload) {
        ...
        return switch (checked.mode()) {
            case RAY_TRACING_SCENE -> {
                ...
                if (attempt instanceof FrameSubmitted submitted) {
    ...
    /**
     * Atomically publishes explicit versioned resources for the generic command lane.
     *
     * <p>2.0 providers must implement this operation. Acceptance only publishes residency; command
     * admission may reference exact generations already accepted here.</p>
     *
     * @param transaction immutable resource publication and retirement batch
     ...
     *
     * @param generation stable identity plus immutable generation
     ...
     *
     * @param transaction prevalidated, strictly ordered command transaction
     * @return typed evidence; acceptance never implies recording or completion
     */
